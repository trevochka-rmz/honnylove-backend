...
const productSchema = Joi.object({
    ...
    stockQuantity:     Joi.number().integer().min(0).optional(),   // Россия (loc=1)
    stockQuantityKg:   Joi.number().integer().min(0).optional(),   // Кыргызстан (loc=4)
});
...
const querySchema = Joi.object({
    ...
    sort: Joi.string()
        .valid('popularity','price_asc','price_desc','rating','new_random','id_desc','newest','relevance')
        .default('id_desc'),
    // ← НОВЫЙ: 'ru' — цены RUB, 'kg' — цены KGS
    region: Joi.string().valid('ru', 'kg').default('ru'),
});
...
const getProductByIdentifier = async (identifier, isAdmin = false, region = 'ru') => {
    const product = await productModel.getProductByIdentifier(identifier, isAdmin, region);
...
const createProduct = async (data, mainImageFile, galleryFiles) => {
    const parsed = {
        ...
        stockQuantityKg:   data.stockQuantityKg ? Number(data.stockQuantityKg) : 0,
...
const updateProduct = async (id, data, mainImageFile, galleryFiles) => {
    ...
    await getProductByIdentifier(id); // проверяем существование
...
const deleteProduct = async (id) => {
    await getProductByIdentifier(id);
...
// Поиск
// ─────────────────────────────────────────────────────────────────
const searchProducts = async (query, region = 'ru') => {
    if (!query) throw new AppError('Не указан поисковый запрос', 400);
    return productModel.searchProducts(query, 1, 20, region);
};
