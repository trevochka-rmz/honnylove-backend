...
// Получить категорию по ID с подкатегориями (только 1 уровень вниз) - для детального просмотра
const getCategoryById = async (id) => {
    // 1. Получаем основную категорию с level и другими полями
    const categoryQuery = `
        SELECT 
            id,
            name,
        ...
        WHERE id = $1
    `; // Убрали AND is_active = true, чтобы админы видели все
    ...
    // 2. Добавляем количество товаров
    const countQuery = `
...
// Получить простую категорию по ID (без детей, для update/delete)
